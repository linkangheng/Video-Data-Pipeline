...
def group_by_keys(
    ...
    """
    current_sample = None
    current_interleave_id = None
    for filesample in data:
        try:
            assert isinstance(filesample, dict)
            fname, value = filesample["fname"], filesample["data"]
            prefix, suffix = keys(fname)
            # NOTE this is a hack to deal with interleave files, the interleave id is the first json file
            # NOTE the other files are renamed to have the same prefix, but the suffix is the original suffix
            # NOTE the `current_sample` is a dict that stores each image with its original filename
            if suffix == "json":
                current_interleave_id = prefix
            elif current_interleave_id is not None and prefix.startswith(current_interleave_id + "-"):
                suffix = prefix[len(current_interleave_id + "-"):] + "." + suffix
                prefix = current_interleave_id
            if trace:
                print(
                    prefix,
                    suffix,
