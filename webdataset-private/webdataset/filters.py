...
def getall(sample_key, keys, default=False):
    if isinstance(keys, str):
        assert " " not in keys
        keys = keys.split(";")
    for valid_key in keys:
        if sample_key.endswith(valid_key):
            return True
    return default
...
def _to_dict(data, *args, handler=reraise_exception, missing_is_error=True, none_is_error=None):
    """Convert dict samples to tuples."""
    if none_is_error is None:
        none_is_error = missing_is_error
    if len(args) == 1 and isinstance(args[0], str) and " " in args[0]:
    ...
    for sample in data:
        try:
            ...
            for sample_key in sample.keys():
                if sample_key.startswith("__"):
                    continue
                for keys in args:
                    if getall(sample_key, keys=keys):
                        result[sample_key] = sample[sample_key]
                        break
                    else:
                        result[sample_key] = None
            if none_is_error and any(x is None for x in result.values()):
                raise ValueError(f"to_dict {args} got {sample.keys()}")
            yield result
        except Exception as exn:
            if handler(exn):
                continue
...
to_dict = pipelinefilter(_to_dict)
